...
// מייצר public/products.json עם 500+ פריטים

type BaseProduct = {
  id: string;            // בסיס ייחודי (ללא גודל)
  label: string;         // שם תצוגה בסיסי (ללא גודל)
  canonical: string;     // שם קאנוני בסיסי (ללא גודל)
  brand?: string;
  tags?: string[];
  synonyms?: string[];
  // אילו פרופילי גדלים להחיל
  sizeProfiles: string[];  // מזהים של פרופילים להלן
};

type SizeVariant = {
  id: string;                 // מזהה פרופיל
  labelSuffix: string;        // למשל "1.5 ליטר", "330 מ״ל", "500 גרם", "6x330 מ״ל"
  canonicalSuffix: string;    // למשל "1.5l", "330ml", "500g", "6x330ml"
  default_size: string;       // יופיע בשדה default_size
  synonyms?: string[];        // קיצורי חיפוש לגדלים
  tags?: string[];            // תוספות מילות מפתח
};
...
// בסיס מוצרים (אפשר להרחיב חופשי; המחולל יעשה מכפלה עם פרופילי גדלים)
const BASE: BaseProduct[] = [
  // משקאות מוגזים — קוקה/ספרייט/פנטה/פפסי
...
// הרחבה ל־500+ ע״י שכפול חכם – נוסיף גם מותגים דומים / טעמים שונים
const EXTRA_BEVERAGES = [
  { id:"sevenup", label:"7UP", canonical:"7up", brand:"7UP" },
  { id:"mountain_dew", label:"מא운טן דיו", canonical:"mountain dew", brand:"PepsiCo" },
...
function buildProducts() {
  ...
  for (const base of BASE) {
    for (const profId of base.sizeProfiles) {
      ...
      products.push({
        ...
        label,
        canonical,
        default_size: prof.default_size,
  ...
  // ודא שיש לפחות 500: אם חסר, נכפיל וריאציות (פשוט)
  while (products.length < 500) {
    const copy = products.slice(0, Math.min(products.length, 100)).map((p, i)=>({
      ...p, id: `${p.id}__dup${i+1}`
    }));
    products.push(...copy);
  }
